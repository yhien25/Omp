		...
		<div class="col-md-9 well">
			...
			<?php
				...
				if(!isset($_GET['search'])){
					$sql = mysql_query("SELECT * FROM tbl_products WHERE prod_category = '$category' AND status = 'Approved' ORDER BY date_posted DESC LIMIT $offset, $rec_limit");
					$sql2 = mysql_query("SELECT * FROM tbl_products WHERE prod_category = '$category' AND status = 'Approved' ORDER BY date_posted DESC");
				}else{
					$search = $_GET['search'];
			...
			<?php }?><table class="table table-bordered table-hover" id="searchable-container">
		<thead>
			<tr>
				<th colspan = "8">
							...
							<div class="input-group">
								Arranged By:
									<select class ="btn btn-default dropdown-toggle" name = "filter">
										...
										<option value = "Relevance">Posted Date</option>
										<option value  = "Cheapest">Price:Cheapest First</option>
										<option value = "mostexpensive">Price:Most Expensive First</option>
									</select>
			...
			<?php
					 ...
					 if($_POST['filter']=="mostexpensive"){
					$sql = mysql_query("SELECT * FROM tbl_products WHERE prod_category = '$category' AND status = 'Approved' ORDER BY price DESC LIMIT $offset, $rec_limit");
					$sql2 = mysql_query("SELECT * FROM tbl_products WHERE prod_category = '$category' AND status = 'Approved' ORDER BY price DESC");
					
				
				 }elseif($_POST['filter']=="Cheapest"){
					  $sql = mysql_query("SELECT * FROM tbl_products WHERE prod_category = '$category' AND status = 'Approved' ORDER BY price ASC LIMIT $offset, $rec_limit");
					$sql2 = mysql_query("SELECT * FROM tbl_products WHERE prod_category = '$category' AND status = 'Approved' ORDER BY price ASC");
				
				}elseif($_POST['filter']=="DatePosted"){
					 $sql = mysql_query("SELECT * FROM tbl_products WHERE prod_category = '$category' AND status = 'Approved' ORDER BY date_posted DESC LIMIT $offset, $rec_limit");
					$sql2 = mysql_query("SELECT * FROM tbl_products WHERE prod_category = '$category' AND status = 'Approved' ORDER BY date_posted DESC");
				...
				if($_POST['filter']=="mostexpensive"){
					$sql = mysql_query("SELECT * FROM tbl_products WHERE status = 'Approved' ORDER BY price DESC LIMIT $offset, $rec_limit");
					$sql2 = mysql_query("SELECT * FROM tbl_products WHERE status = 'Approved' ORDER BY price DESC");
					
				
				 }elseif($_POST['filter']=="Cheapest"){
					  $sql = mysql_query("SELECT * FROM tbl_products WHERE status = 'Approved' ORDER BY price ASC LIMIT $offset, $rec_limit");
					$sql2 = mysql_query("SELECT * FROM tbl_products WHERE  status = 'Approved' ORDER BY price ASC");
				
				}elseif($_POST['filter']=="DatePosted"){
					 $sql = mysql_query("SELECT * FROM tbl_products WHERE status = 'Approved' ORDER BY date_posted DESC LIMIT $offset, $rec_limit");
					$sql2 = mysql_query("SELECT * FROM tbl_products WHERE status = 'Approved' ORDER BY date_posted DESC");
				...
				<div class=  "col-md-4" id ="prod"> 
					<figure>
						...
						<a href = "product_info.php?&category=<?php echo $category; ?>&id=<?php echo $row['id']; ?>">
										<img src = "images/products/<?php echo $row['picture1']; ?>" height ="100%"> </a>
						</div>
						<figcaption>
							<h4>
